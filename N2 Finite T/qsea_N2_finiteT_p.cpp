...
class QSEA{
    public:
        ...
        {   
            ...
            m12 = params["m12"].get<double>();
            m22 = params["m22"].get<double>();
            lambda1 = params["lambda1"].get<double>();
            lambda2 = params["lambda2"].get<double>();
            lambda12 = params["lambda12"].get<double>();
            alpha = params["alpha"].get<double>();

            kStart = params["params"]["kStart"].get<double>();
            phi1Steps = params["params"]["phi1Steps"].get<int>();
            phi2Steps = params["params"]["phi2Steps"].get<int>();
            vector<double> phi1Limits = params["params"]["phi1Range"].get<vector<double>>();
            vector<double> phi2Limits = params["params"]["phi2Range"].get<vector<double>>();
            ...
            phi1Range[0] = phi1Limits[0];
            phi1Range[1] = phi1Limits[1];
            phi2Range[0] = phi2Limits[0];
            phi2Range[1] = phi2Limits[1];
            ...
            for(int i=0;i<phi1Steps;i++){
                double phi1;
                phi1 = phi1Range[0]+i*dphi1;
                fieldValues.push_back(phi1);
                treeLevelPot.push_back(u[i][zeroJ]);
            }
...
#pragma omp critical
            ...
            for(int i=0;i<phi1Steps;i++){
                finiteTempOutput["pot"].push_back(u[i][zeroJ]);
            }
...
#pragma omp critical
    ...
    private:
        ...
        /*Cut off for when input for propagator solutions are small and another solution is preferred.
        ...
        bool nextStepRK45(double **u, double k, double **k1, double **k2, double **k3, double **k4, double **k5, double **k6){
            ...
            if(TE>epsilon){
                ...
                for(int i=0;i<phi1Steps;i++){
                    for(int j=0;j<phi2Steps;j++){
                        u[i][j]+= dk*(CH[0]*k1[i][j]+CH[1]*k2[i][j]+CH[2]*k3[i][j]+CH[3]*k4[i][j]+CH[4]*k5[i][j]+CH[5]*k6[i][j]);;
                    }
                }
                dk=dknew;
                for(int i=0;i<phi1Steps;i++){
